...
var amountPriceDay = document.getElementById('amountPriceDay');
...
dbExpensesRef.on('value', function(snapshot) {
  var priceAmount = 0;
    ...
    var dataResponse = child.val();
    if(
      date.getDate() == dataResponse.day
      && date.getMonth() + 1 == dataResponse.month
      && date.getFullYear() == dataResponse.year) {
        priceAmount += parseFloat(dataResponse.price);
    }
	});
  amountPriceDay.innerHTML = 'R$ ' + priceAmount;
